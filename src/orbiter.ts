...
function init() {
  ...
  headerTitle.innerHTML =
    'Orbital rocket simulation demo - powered by <a href="http://threejs.org" target="_blank">three.js</a>';
  ...
  const meshVertices = [];
  for (let x = -10; x <= 10; x++) meshVertices.push(-10, x, 0, 10, x, 0);
  for (let x = -10; x <= 10; x++) meshVertices.push(x, -10, 0, x, 10, 0);
  meshGeometry.setAttribute(
    "position",
    new THREE.BufferAttribute(new Float32Array(meshVertices), 3)
  );
  ...
  function addAxis(axisVector: THREE.Vector3, color: number) {
    ...
    axisXGeometry.setAttribute(
      "position",
      new THREE.BufferAttribute(
        new Float32Array([0, 0, 0, axisVector.x, axisVector.y, axisVector.z]),
        3
      )
    );
  ...
  timescaleControl = new TimeScaleControl(function (scale) {
    return gameState.setTimeScale(scale);
  });
  ...
  class SpeedControl {
    ...
    setSpeed() {
      if (gameState.select_obj) {
        ...
        else this.valueElement.innerHTML = value.toFixed(4) + "km/s";
      } else this.valueElement.innerHTML = "";
  ...
  statsControl = new StatsControl(settingsControl, function () {
    return gameState.getSelectObj();
  });
  ...
  class AltitudeControl {
    ...
    setText(value: number) {
      let text;
      if (value < 1e5) text = value.toFixed(4) + "km";
      else if (value < 1e8) text = (value / 1000).toFixed(4) + "Mm";
      else text = (value / AU).toFixed(4) + "AU";
  ...
  scenarioSelectorControl = new ScenarioSelectorControl(
    ...
    function (throttle) {
      throttleControl.setThrottle(throttle);
    },
    function () {
      gameState.resetTime();
    },
    function (msg) {
      messageControl.setText(msg);
    },
    function () {
      [saveControl, loadControl].map(function (control) {
  ...
  saveControl = new SaveControl(
    function () {
      return gameState.serializeState();
    },
    function (msg) {
      messageControl.setText(msg);
    },
    function () {
      [scenarioSelectorControl, loadControl].map(function (control) {
        control.setVisible(false);
      }); // Mutually exclusive
    }
  );
  ...
  gameState.onStateLoad = () =>
    throttleControl.setThrottle(gameState.select_obj.throttle);
  ...
  loadControl = new LoadControl(
    ...
    function () {
      [scenarioSelectorControl, saveControl].map(function (control) {
        control.setVisible(false);
      }); // Mutually exclusive
    }
  );
  ...
  window.addEventListener("beforeunload", function () {
    localStorage.setItem(
      "WebGLOrbiterAutoSave",
      JSON.stringify(gameState.serializeState())
    );
  });
...
function render() {
  ...
  camera.near = Math.min(
    1,
    cameraControls.target.distanceTo(camera.position) / 10
  );
  camera.updateProjectionMatrix();
  ...
  gameState.universe.update(
    settings.center_select,
    viewScale,
    settings,
    camera,
    windowHalfX,
...
function onKeyDown(event: KeyboardEvent) {
  ...
  if (code === 109 || code === 189) timescaleControl.decrement();
  if (code === 16) accelerate = true;
  if (code === 17) decelerate = true;
}

function onKeyUp(event: KeyboardEvent) {
  // Annoying browser incompatibilities
  const code = event.which || event.keyCode;
  if (code === 16) accelerate = false;
  if (code === 17) decelerate = false;
}
